...
export function ClassAssignment({ session, onNavigateToComparison }: ClassAssignmentProps) {
  ...
  const loadClasses = async () => {
    try {
      ...
      console.error("Error memuatkan kelas:", error);
    }
  };
  ...
  const loadStudents = async () => {
    try {
      ...
      console.error("Error memuatkan Pelajar:", error);
    }
  };
  ...
  const loadAssignedStudents = async (classId: string) => {
    try {
      ...
      console.error("Error memuatkan pelajar yang ditugaskan:", error);
    }
  };
  ...
  const createClass = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.error(error.error || "Gagal mencipta kelas");
      }
    } catch (error) {
      console.error("Error cipta kelas:", error);
      toast.error("Gagal mencipta kelas");
    } finally {
      setLoading(false);
    }
  };
  ...
  const assignStudentsToClass = async () => {
    if (!selectedClass) {
      toast.error("Sila pilih kelas");
      return;
    }

    if (selectedStudents.size === 0) {
      toast.error("Sila pilih sekurang-kurangnya seorang pelajar");
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        toast.success(`${studentIds.length} Pelajar Berjaya ditempatkan`);
        setSelectedStudents(new Set());
        await loadAssignedStudents(selectedClass);
      } else {
        const error = await response.json();
        toast.error(error.error || "Gagal menempatkan Pelajar");
      }
    } catch (error) {
      console.error("Error menempatkan Pelajar:", error);
      toast.error("Gagal menempatkan Pelajar");
    } finally {
      setLoading(false);
    }
  };
  ...
  const removeStudentFromClass = async (studentId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success("Pelajar dikeluarkan dari kelas");
        await loadAssignedStudents(selectedClass);
      } else {
        const error = await response.json();
        toast.error(error.error || "Gagal mengeluarkan Pelajar");
      }
    } catch (error) {
      console.error("Error mengeluarkan Pelajar:", error);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-br from-indigo-600 via-blue-600 to-purple-600 rounded-2xl p-8 shadow-2xl relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-3">
            ...
            <h2 className="text-3xl text-white">Pengurusan Kelas</h2>
          </div>
          <p className="text-white/90 ml-[60px] max-w-2xl">
            Bina komuniti pembelajaran anda. Susun pelajar ke dalam kelas dan buka kunci pandangan berkumpulan yang hebat.
          </p>
        </div>
      </div>
        ...
        <CardHeader className="bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 border-b border-slate-200/60">
          <div className="space-y-1.5">
            ...
            <CardDescription className="text-slate-600 ml-[52px]">
              Cipta ruang pembelajaran baharu atau pilih kelas sedia ada untuk bermula
            </CardDescription>
          </div>
        </CardHeader>
          ...
          <div className="space-y-3">
            ...
            <Select value={selectedClass} onValueChange={setSelectedClass}>
              <SelectTrigger id="selectClass" className="border-slate-200 focus:border-indigo-400 focus:ring-indigo-400/20 h-11 rounded-lg transition-all">
                <SelectValue placeholder="Pilih kelas untuk diuruskan..." />
              </SelectTrigger>
              <SelectContent>
                {classes.length === 0 ? (
                  <SelectItem value="no-classes" disabled>Tiada kelas tersedia - buat satu di atas</SelectItem>
                ) : (
                  classes.map((cls) => (
            ...
            {selectedClass && (
              <div className="flex items-center gap-2.5 text-sm bg-gradient-to-r from-indigo-50 to-blue-50 p-4 rounded-xl border border-indigo-100">
                ...
                <div>
                  <p className="text-slate-500 text-xs">Sedang mengurus</p>
        ...
        <Card className="shadow-xl rounded-2xl border border-slate-200/60 overflow-hidden hover:shadow-2xl transition-all duration-300">
          <CardHeader className="bg-gradient-to-br from-green-50 via-emerald-50 to-teal-50 border-b border-slate-200/60">
            <div className="space-y-1.5">
              ...
              <CardDescription className="text-slate-600 ml-[52px]">
                Cari dan pilih pelajar untuk menyertai <span className="text-emerald-600">{classes.find(c => c.id === selectedClass)?.name}</span>
              </CardDescription>
            </div>
          </CardHeader>
          <CardContent className="space-y-4 p-6 bg-white dark:bg-gray-800">
            {/* Search Bar */}
            <div className="relative">
              <Search className="absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 text-slate-400" />
              <Input
                placeholder="Cari mengikut ID atau Nama Pelajar..."
                value={searchTerm}
                    ...
                    <TableRow>
                      <TableCell colSpan={3} className="text-center text-gray-500 py-12">
                        <div className="flex flex-col items-center gap-2">
                          ...
                          <p>
                            {searchTerm
                              ? "Tiada pelajar yang sepadan dengan carian anda ditemui"
                              : "Semua pelajar telah ditugaskan ke kelas ini"}
                          </p>
                        </div>
                      </TableCell>
                    </TableRow>
            ...
            {selectedStudents.size > 0 && (
              <div className="flex items-center justify-between bg-gradient-to-r from-green-50 to-emerald-50 p-4 rounded-xl border border-green-200">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <p className="text-sm text-slate-500">Dipilih</p>
                    <p className="text-green-700">{selectedStudents.size} Pelajar</p>
                  </div>
                </div>
                <Button
                  variant="ghost"
            ...
            >
              ...
              Daftar {selectedStudents.size > 0 ? `${selectedStudents.size} ` : ''}Pelajar ke Kelas
            </Button>
          </CardContent>
        </Card>
      )}
          ...
          <CardHeader className="bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 border-b border-slate-200/60">
            <div className="flex items-start justify-between">
              <div className="space-y-1.5">
                ...
                <CardDescription className="text-slate-600 ml-[52px]">
                  {assignedStudents.length} {assignedStudents.length === 1 ? 'student' : 'students'} sedang dalam {classes.find(c => c.id === selectedClass)?.name}
                </CardDescription>
              </div>
                    ...
                    <TableRow>
                      <TableCell colSpan={3} className="text-center text-gray-500 py-12">
                        <div className="flex flex-col items-center gap-2">
                          ...
                          <p>Tiada pelajar yang ditugaskan lagi</p>
                          <p className="text-sm">Gunakan bahagian di atas untuk menambah pelajar</p>
                        </div>
                      </TableCell>
                    </TableRow>
                  ) : (
      ...
      <Card className="shadow-xl rounded-2xl border border-slate-200/60 overflow-hidden bg-gradient-to-br from-orange-50 via-pink-50 to-purple-50 hover:shadow-2xl transition-all duration-300">
        <CardHeader className="border-b border-slate-200/60">
          <div className="space-y-1.5">
            ...
            <CardDescription className="text-slate-600 ml-[52px]">
              Manfaatkan kuasa analitik perbandingan merentasi semua kelas anda
            </CardDescription>
          </div>
        </CardHeader>
        <CardContent className="p-6">
          <div className="space-y-4">
            <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm p-5 rounded-xl border border-orange-100 dark:border-orange-900 shadow-sm">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  <p className="text-slate-700 leading-relaxed">
                    Selami metrik prestasi secara mendalam. Bandingkan skor purata, kenal pasti pelajar berisiko lebih awal dan temui trend pembelajaran dengan visualisasi interaktif yang cantik.
                  </p>
                </div>
              </div>
            </div>
            ...
            >
              ...
              Papan Pemuka Perbandingan Kelas Terbuka
            </Button>
            {classes.length === 0 && (
              <p className="text-sm text-gray-500 text-center">
                Cipta sekurang-kurangnya satu kelas untuk mengakses analitik
              </p>
            )}
          </div>
        </CardContent>
      ...
      <AlertDialog open={!!studentToRemove} onOpenChange={() => setStudentToRemove(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Keluarkan Pelajar daripada Kelas?</AlertDialogTitle>
            <AlertDialogDescription>
              Ini akan mengeluarkan pelajar daripada kelas. Rekod kemajuan pelajar tidak akan dipadamkan.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>

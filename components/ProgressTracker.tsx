...
// Helper function to translate assessment types to Malay
...
export function ProgressTracker({
  ...
  const fetchProgress = async (studentId: string) => {
    ...
    try {
      ...
      const msg = (error?.message || "")
        ...
        ? "Unauthorized: Sila login sebagai cikgu atau pelajar"
        : error?.message || "Gagal memuatkan data progress";
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  ...
  const fetchAllProgress = async () => {
    ...
    try {
      ...
      const msg = (error?.message || "")
        .toLowerCase()
        .includes("unauthorized")
        ? "Unauthorized: Cikgu/Admin sahaja"
        : error?.message || "Gagal memuatkan overview progress";
      toast.error(msg);
    } finally {
      setLoading(false);
    }
  ...
  const AnalyticsCharts = ({
    ...
    const normalizeType = (t?: string) => {
      ...
      if (v === "general") return translateAssessmentType("Fundamentals");
      if (v === "exam") return translateAssessmentType("Prototyping");
      return translateAssessmentType(t || "Fundamentals");
    };
    const typeCounts = (() => {
    ...
    return (
      <div className="grid gap-6 md:grid-cols-3">
        <div className="bg-white rounded-xl p-6 border border-slate-200/60 shadow-sm hover:shadow-md transition-all">
          <h4 className="text-gray-900 mb-4">Prestasi Topik</h4>
        ...
        <div className="bg-white rounded-xl p-6 border border-slate-200/60 shadow-sm hover:shadow-md transition-all">
          <h4 className="text-gray-900 mb-4">Trend Kemajuan</h4>
          <div className="flex items-center justify-center">
            <LineChart
              width={360}
              height={240}
        ...
        <div className="bg-white rounded-xl p-6 border border-slate-200/60 shadow-sm hover:shadow-md transition-all">
          <h4 className="text-gray-900 mb-4">Jenis Penilaian</h4>
          <div className="flex items-center justify-center">
            <PieChart
              width={360}
              height={240}
  ...
  const handleAddProgress = async (
    ...
    try {
      ...
      toast.success("Rekod kemajuan berjaya ditambah");
      setDialogOpen(false);
      ...
      toast.error(error.message || "Gagal menambah kemajuan");
    }
  };
  ...
  const handleEditProgress = async (
    ...
    try {
      ...
      if (!response.ok)
        throw new Error("Gagal mengemaskini kemajuan");

      toast.success("Rekod kemajuan berjaya dikemaskini");
      ...
      console.error("Error mengemaskini kemajuan:", error);
      toast.error(error.message || "Gagal mengemaskini kemajuan");
    }
  };
  ...
  const handleDeleteProgress = async () => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error("Gagal untuk delete rekod kemajuan");

      toast.success("Rekod kemajuan Berjaya delete!");
      ...
      console.error("Error delete kemajuan:", error);
      toast.error(error.message || "Gagal untuk delete kemajuan");
    }
  };
  ...
  const saveTeacherNote = (studentId: string, note: string) => {
    ...
    toast.success("Nota Cikgu disimpan");
  ...
  const exportProgressToPDF = () => {
    ...
    const reportTitle = isOverview
      ? "Laporan Kemajuan Keseluruhan Pelajar"
    ...
    doc.setFontSize(15);
    doc.setTextColor(accentColor.r, accentColor.g, accentColor.b);
    doc.text("Ringkasan Sekilas", marginX, y);
    y += 22;

    const summaryTableData: Array<{ label: string; value: string }> = [];
    if (!isOverview && studentIdInput) {
      summaryTableData.push({ label: "ID/E-mel Pelajar", value: studentIdInput });
    }
    summaryTableData.push({ label: "Skor Average", value: `${avg}%` });
    summaryTableData.push({ label: `Kadar Lulus (>=${threshold}%)`, value: `${passPct}%` });
    summaryTableData.push({ label: "Jumlah Rekod", value: `${filteredData.length}` });
    summaryTableData.push({ label: "Topik Dibincangkan", value: `${topics}` });
    summaryTableData.push({ label: "Pelajar Berisiko", value: `${atRisk}` });
    ...
    doc.setFontSize(15);
    doc.setTextColor(accentColor.r, accentColor.g, accentColor.b);
    doc.text("Rekod Kemajuan Terperinci", marginX, y);
    y += 25;
    ...
    filteredData.forEach((record, rowIndex) => {
      const cells = [
        ...
        record.assessmentType
          ? translateAssessmentType(record.assessmentType)
          : "Asas",
        `${record.score}%`,
        record.notes || ""
      ];
    ...
    doc.setFontSize(10);
    doc.setTextColor(textGray.r, textGray.g, textGray.b);
    doc.text("— Akhir Laporan —", pageWidth / 2, y, { align: "center" });
    ...
    return (
      <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-3xl text-white mb-2">
                Analisis Kemajuan Pelajar
              </h2>
              <p className="text-indigo-100">
                Pantau prestasi, jejaki trend dan sokong kejayaan pelajar
              </p>
            </div>
            <Dialog
              open={dialogOpen}
            ...
            <DialogContent className="max-w-2xl">
              <DialogHeader className="pb-4 border-b border-gray-100">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <DialogTitle className="text-2xl">Tambah Kemajuan Pelajar 📝</DialogTitle>
                    <p className="text-sm text-gray-600 mt-1">
                      Rekodkan pencapaian atau keputusan penilaian baharu
                    </p>
                  </div>
                </div>
              </DialogHeader>
              ...
              >
                <div className="space-y-2.5">
                  ...
                  <Input
                    id="studentId"
                    name="studentId"
                    placeholder="Masukkan ID pengguna pelajar..."
                    required
                    className="border-gray-200 focus:border-indigo-400 focus:ring-indigo-400/20 h-11 rounded-lg"
                  />
                  <p className="text-xs text-gray-500 flex items-center gap-1.5">
                    💡 Tip: Cari ID pelajar dalam profil mereka atau papan pemuka analitik
                  </p>
                </div>
                <div className="space-y-2.5">
                ...
                <div className="space-y-2.5">
                  ...
                  <Textarea
                    id="notes"
                    name="notes"
                    placeholder="Tambahkan maklum balas atau pemerhatian yang menggalakkan..."
          ...
          <DialogContent className="max-w-2xl">
            <DialogHeader className="pb-4 border-b border-gray-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <DialogTitle className="text-2xl">Ubah Rekod Kemajuan ✏️</DialogTitle>
                  <p className="text-sm text-gray-600 mt-1">
                    Kemas kini butiran penilaian pelajar dan maklum balas
                  </p>
                </div>
              </div>
            </DialogHeader>
            ...
            >
              ...
              <div className="space-y-2.5">
                ...
                <Textarea
                  ...
                  placeholder="Kemaskini maklum balas atau pemerhatian..."
                  className="border-gray-200 focus:border-amber-400 focus:ring-amber-400/20 rounded-lg min-h-[100px]"
                />
              </div>
              <div className="flex gap-3 justify-end pt-4 border-t border-gray-100">
          ...
          <AlertDialogContent className="max-w-lg">
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  <AlertDialogTitle className="text-2xl mb-2">
                    Delete Rekod Kemajuan? 🗑️
                  </AlertDialogTitle>
                  <AlertDialogDescription className="text-gray-600 leading-relaxed">
                    Ini akan memadamkan rekod kemajuan ini daripada sejarah pelajar secara kekal. 
                    Tindakan ini <span className="text-red-600 font-medium">tidak boleh dibatalkan</span>.
                  </AlertDialogDescription>
                </div>
              </div>
            </AlertDialogHeader>
            <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-lg p-4 border border-amber-200/60">
              <p className="text-sm text-amber-900 flex items-start gap-2">
                ...
                <span>
                  <strong className="font-medium">Fikir dua kali:</strong> Pertimbangkan untuk mengedit rekod sebaliknya jika anda hanya perlu mengemas kini butirannya.
                </span>
              </p>
            </div>
        ...
        >
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Nota Cikgu</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <Textarea
                  id="teacher-note"
                  placeholder="Masukkan pemerhatian, cadangan atau item tindakan..."
                  defaultValue={
                    currentStudentId
            ...
            <div className="space-y-3 mt-4">
              {getAtRiskStudents().length === 0 ? (
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p>Semua pelajar membuat persembahan di atas {threshold}% ambang!</p>
                </div>
              ) : (
                <>
                  <p className="text-sm text-gray-600 mb-4">
                    Berikutnya adalah {getAtRiskStudents().length} Pelajar yang{getAtRiskStudents().length > 1 ? 's are' : ' is'} sedang beraksi di bawah {threshold}% ambang batas dan mungkin mendapat manfaat daripada sokongan tambahan.
                  </p>
                  {getAtRiskStudents()
              ...
              <div className="space-y-1.5">
                <CardTitle className="flex items-center gap-3">
                  ...
                  <span className="bg-gradient-to-r from-indigo-600 to-blue-600 bg-clip-text text-transparent">
                    Papan Pemuka Kemajuan Pelajar
                  </span>
                </CardTitle>
                <CardDescription className="text-slate-600 ml-[52px]">
                  Jejaki perjalanan individu atau terokai gambaran yang lebih besar
                </CardDescription>
              </div>
            </div>
          </CardHeader>
          ...
          <CardContent className="p-6 bg-white">
            <div className="flex gap-3 items-center flex-wrap">
              <div className="flex-1 min-w-[200px]">
                <Input
                  placeholder="Masukkan ID Pelajar untuk bermula..."
                ...
                <div className="space-y-1.5">
                  <CardTitle className="flex items-center gap-3">
                    ...
                    <span className="bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent">
                      Tapis & Perhalusi Keputusan
                    </span>
                  </CardTitle>
                  <CardDescription className="text-slate-600 ml-[52px]">
                    Perhalusi pandangan anda untuk menumpukan pada perkara yang paling penting
                  </CardDescription>
                </div>
              </div>
            </CardHeader>
                ...
                <div className="bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 rounded-2xl p-8 shadow-lg border border-indigo-100">
                  <div className="flex items-start justify-between mb-6">
                    <div>
                      ...
                      <p className="text-gray-600">
                        Gambaran ringkas kemajuan & pencapaian pembelajaran
                      </p>
                    </div>
                    {getAtRiskStudents().length > 0 && (
                      <Badge
                      ...
                      >
                        ...
                        {getAtRiskStudents().length} perlu sokongan
                      </Badge>
                    )}
                  </div>
                  ...
                  <div className="grid gap-5 md:grid-cols-3 mb-6">
                    <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-all">
                      ...
                      <p className="text-gray-600 mb-1">Average Kelas</p>
                      <p className="text-sm text-gray-500">
                        {calculateAverageScore() >= 80
                          ? "🌟 Kerja yang Bagus!"
                          : calculateAverageScore() >= 70
                            ? "👍 Kemajuan yang Mantap"
                            : "💪 Terdapat ruang untuk memperbaiki diri"}
                      </p>
                    </div>
                    ...
                    <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-all">
                      ...
                      <p className="text-gray-600 mb-1">Penilaian</p>
                      <p className="text-sm text-gray-500">
                        {filteredData.length > 20
                          ? "📚 Penglibatan yang hebat"
                    ...
                    <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-all">
                      ...
                      <p className="text-gray-600 mb-1">Topik yang Dibincangkan</p>
                      <p className="text-sm text-gray-500">
                  ...
                  <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-gray-900">Kadar Kejayaan</p>
                          <p className="text-sm text-gray-500">
                            Pelajar mengeskor {threshold}% atau lebih
                          </p>
                        </div>
                      </div>
                      <div className="flex items-center gap-3">
                    ...
                    {getTopicExtremes().highest ? (
                      <div className="bg-white rounded-xl p-5 border border-emerald-100 shadow-sm">
                        ...
                        <p className="text-sm text-emerald-700 mt-3">
                          ✨ Pelajar sedang menguasai bidang ini — teruskan momentum hebat ini!
                        </p>
                      </div>
                    ) : (
                      <div className="bg-white rounded-xl p-5 border border-emerald-100 text-emerald-700">
                        Tiada data tersedia lagi
                      </div>
                    )}
                  </div>
                    ...
                    <div className="flex items-start gap-4 mb-4">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-orange-700">
                          Peluang untuk mengukuhkan kemahiran
                        </p>
                      </div>
                    </div>
                    {getTopicExtremes().lowest ? (
                      <div className="bg-white rounded-xl p-5 border border-orange-100 shadow-sm">
                        ...
                        <p className="text-sm text-orange-700 mt-3">
                          🎯 Pertimbangkan sumber tambahan, latihan amali atau sesi ulasan di sini.
                        </p>
                      </div>
                    ) : (
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-rose-700 mb-4">
                          {getAtRiskStudents().length} Pelajar
                          {getAtRiskStudents().length > 1 ? "s are" : " is"}{" "}
                          currently below the {threshold}% ambang batas. Semak tab Pelajar untuk menyemak kemajuan mereka dan memberikan sokongan peribadi.
                        </p>
              ...
              <TabsContent value="students" className="space-y-6 mt-6">
                <div className="bg-gradient-to-br from-slate-50 via-white to-blue-50 rounded-2xl p-8 border border-slate-200/60 shadow-sm">
                  <div className="mb-6">
                    ...
                    <p className="text-gray-600">
                      Pandangan yang jelas tentang perjalanan pembelajaran dan kemajuan setiap pelajar dari semasa ke semasa
                    </p>
                  </div>
                  <div className="space-y-4">
                      {getStudentAverages()
              ...
              <TabsContent value="topics" className="space-y-6 mt-6">
                <div className="bg-gradient-to-br from-purple-50 via-white to-pink-50 rounded-2xl p-8 border border-purple-200/60 shadow-sm">
                  <div className="mb-6">
                    ...
                    <p className="text-gray-600">
                      Memahami di mana pelajar cemerlang dan di mana mereka memerlukan lebih banyak sokongan
                    </p>
                  </div>
                  <div className="space-y-4">
                      {getTopicProgress()
                            ...
                            >
                              ...
                              <p className="text-sm text-gray-600 mt-4">
                                {mastery.level === "Expert"
                                  ? "✨ Pelajar benar-benar telah menguasai topik ini"
                                  : mastery.level === "Proficient"
                                    ? "🎯 Pemahaman yang kukuh ditunjukkan oleh pelajar"
                                    : mastery.level === "Developing"
                                      ? "📈 Pelajar menunjukkan kemajuan yang stabil"
                                      : "💡 Topik ini memerlukan lebih perhatian dan sokongan"}
                              </p>
                            </div>
                          );
                        })}
              ...
              <TabsContent value="charts" className="space-y-6 mt-6">
                <div className="bg-gradient-to-br from-teal-50 via-white to-cyan-50 rounded-2xl p-8 border border-teal-200/60 shadow-sm">
                  <div className="mb-6">
                    ...
                    <p className="text-gray-600">
                      Carta dan graf interaktif yang menghidupkan data anda
                    </p>
                  </div>
                  <AnalyticsCharts
        ...
        {!loading && progressData.length > 0 && (
          <div className="bg-gradient-to-br from-slate-50 via-white to-indigo-50 rounded-2xl p-8 border border-slate-200/60 shadow-sm">
            <div className="mb-6">
              ...
              <p className="text-gray-600">
                Pandangan kronologi tentang peristiwa penting dan pencapaian pembelajaran 📚
              </p>
            </div>
        ...
        {!loading && filteredData.length > 0 && (
          <div className="bg-gradient-to-br from-purple-50 via-white to-pink-50 rounded-2xl p-8 border border-purple-200/60 shadow-sm">
            <div className="mb-6">
              ...
              <p className="text-gray-600">
                Pecahan terperinci semua rekod penilaian - klik pengepala lajur untuk menyusun 🎯
              </p>
            </div>
              ...
              <div className="overflow-x-auto">
                <Table>
                        ...
                        <TableRow key={i} className="hover:bg-purple-50/30 transition-colors">
                          ...
                          <TableCell>
                            <Badge variant="outline" className="bg-white">
                              {translateAssessmentType(r.assessmentType || "Fundamentals")}
                            </Badge>
                          </TableCell>
                          <TableCell>
                  ...
                  <TableCaption className="text-gray-500 py-4 bg-gradient-to-r from-purple-50/30 to-pink-50/30">
                    💡 Tip: Klik mana-mana pengepala lajur untuk menyusun jadual
                  </TableCaption>
                </Table>
              </div>
            </div>
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-3xl text-gray-900">Kemajuan Saya</h2>
        <p className="text-gray-600 mt-1">
          Jejaki perjalanan pembelajaran dan pencapaian anda
        </p>
      </div>

      {/* Chibi Character */}
      <ChibiCharacter message="Hi!! Mari lihat kemajuan anda" />
      ...
      {progressData.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-gray-600">
              Tiada rekod kemajuan lagi. Selesaikan beberapa penilaian
        ...
        <>
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Tapis mengikut topik, jenis penilaian, atau julat tarikh
              </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <div className="space-y-2">
                  ...
                  >
                    <SelectTrigger id="date-filter">
                      <SelectValue placeholder="Semua Masa" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="all">
                        Semua Masa
          ...
          {progressData.length > 0 && (
            <div className="bg-gradient-to-br from-indigo-50 via-white to-purple-50 rounded-2xl p-8 border border-indigo-200/60 shadow-sm">
              <div className="mb-6">
                ...
                <p className="text-gray-600">
                  Carta interaktif yang menunjukkan corak dan trend pembelajaran anda 📈
                </p>
              </div>
              <AnalyticsCharts
                records={filteredData}
          ...
          <div className="grid gap-5 md:grid-cols-3">
            <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-6 shadow-sm border border-blue-200/60 hover:shadow-md transition-all">
              ...
              <p className="text-gray-900 mb-1">Skor Purata</p>
            ...
            <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-6 shadow-sm border border-purple-200/60 hover:shadow-md transition-all">
              ...
              <p className="text-gray-900 mb-1">Jumlah Rekod</p>
              <p className="text-sm text-gray-600">
                {filteredData.length > 10
                  ? "📚 Penglibatan hebat"
                  : "📖 Sedang membina"}
            ...
            <div className="bg-gradient-to-br from-emerald-50 to-green-50 rounded-2xl p-6 shadow-sm border border-emerald-200/60 hover:shadow-md transition-all">
              ...
              <p className="text-gray-900 mb-1">Topik Diliputi</p>
              <p className="text-sm text-gray-600">
                {getTopicProgress().length >= 5
                  ? "🎯 Kepelbagaian hebat"
                  : "📍 Meneroka"}
          ...
          <div className="bg-gradient-to-br from-amber-50 via-white to-orange-50 rounded-2xl p-8 border border-amber-200/60 shadow-sm">
            <div className="mb-6">
              <h3 className="text-gray-900 mb-2">
                Kemajuan mengikut Topik
              </h3>
              <p className="text-gray-600">
                Prestasi anda merentasi pelbagai bidang pembelajaran 📊
              </p>
            </div>
            <div className="space-y-4">
          ...
          <div className="bg-gradient-to-br from-teal-50 via-white to-cyan-50 rounded-2xl p-8 border border-teal-200/60 shadow-sm">
            <div className="mb-6">
              ...
              <p className="text-gray-600">
                Penilaian dan kemajuan pembelajaran terkini anda ⚡
              </p>
            </div>
            <div className="space-y-3">
              {filteredData

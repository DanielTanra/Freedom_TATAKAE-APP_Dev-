...
app.get("/make-server-d59960c4/debug/routes", (c) => {
  return c.json({
    ...
    availableRoutes: [
      ...
      "PUT /videos/:id",
      "POST /videos/delete",
      "POST /videos/student-progress",
      "POST /videos/progress",
      "POST /videos/save-progress",
...
app.post(
  ...
  async (c) => {
    try {
      ...
      const notes = `Selesai ${assessment.title} - ${score}/${totalQuestions} betul`;
...
// List all videos
app.post("/make-server-d59960c4/videos/list", async (c) => {
  try {
    const user = await verifyAuth(c.req.header("Authorization"));
    if (!user) return c.json({ error: "Unauthorized" }, 401);

    // Get all videos from kv store using getByPrefix
...
app.post("/make-server-d59960c4/videos/upload", async (c) => {
  try {
    const user = await verifyAuth(c.req.header("Authorization"));
    if (!user || user.user_metadata.role !== "teacher") {
      return c.json({ error: "Unauthorized - Teachers only" }, 401);
    ...
    const video = {
      id: videoId.replace("video:", ""),
      title: videoData.title,
      description: videoData.description,
      url: videoData.url,
      duration: videoData.duration,
      topic: videoData.topic,
      category: videoData.category || 'Lain-lain',
      uploadedBy: user.user_metadata.name || user.email,
...
// Update video
app.put("/make-server-d59960c4/videos/:id", async (c) => {
  try {
    const user = await verifyAuth(c.req.header("Authorization"));
    if (!user || user.user_metadata.role !== "teacher") {
      return c.json({ error: "Unauthorized - Teachers only" }, 401);
    ...
    const existingVideo = await kv.get(`video:${videoId}`);
...
// Delete video
app.post("/make-server-d59960c4/videos/delete", async (c) => {
  try {
    const user = await verifyAuth(c.req.header("Authorization"));
    if (!user || user.user_metadata.role !== "teacher") {
      return c.json({ error: "Unauthorized - Teachers only" }, 401);
    ...
    // Also delete all student progress for this video
...
// Get student progress for videos (teacher view)
app.post("/make-server-d59960c4/videos/student-progress", async (c) => {
  try {
    const user = await verifyAuth(c.req.header("Authorization"));
    if (!user || user.user_metadata.role !== "teacher") {
      return c.json({ error: "Unauthorized - Teachers only" }, 401);
    ...
    // Get all student progress for this video
    ...
    for (const progress of progressData) {
      if (progress && progress.videoId === videoId) {
        // Get student info
        const { data: studentData } = await supabase.auth.admin.getUserById(progress.studentId);
        if (studentData?.user) {
          allProgress.push({
            studentId: progress.studentId,
            studentName: studentData.user.user_metadata?.name || studentData.user.email,
            watchStatus: progress.completed ? 'completed' : (progress.watchPercentage > 0 ? 'in-progress' : 'not-started'),
            watchPercentage: progress.watchPercentage || 0,
            understandingPercentage: progress.understandingPercentage || 0,
...
// Get student's own video progress
app.post("/make-server-d59960c4/videos/progress", async (c) => {
  try {
    const user = await verifyAuth(c.req.header("Authorization"));
    if (!user) return c.json({ error: "Unauthorized" }, 401);
    ...
    // Students can only get their own progress
    if (user.user_metadata.role !== "teacher" && user.id !== studentId) {
      return c.json({ error: "Unauthorized" }, 401);
    }
    ...
    return c.json({ error: "Failed to get video progress" }, 500);
  }
});

// Save student video progress
app.post("/make-server-d59960c4/videos/save-progress", async (c) => {
  try {
    const user = await verifyAuth(c.req.header("Authorization"));
    if (!user) return c.json({ error: "Unauthorized" }, 401);
    ...
    const progressKey = `video-progress:${studentId}:${progress.videoId}`;
    await kv.set(progressKey, {
      studentId,
      videoId: progress.videoId,
      watchPercentage: progress.watchPercentage,
      understandingPercentage: progress.understandingPercentage,
...
app.post("/make-server-d59960c4/videos/:id/download", async (c) => {
  try {
    const user = await verifyAuth(c.req.header("Authorization"));
    if (!user) return c.json({ error: "Unauthorized" }, 401);
    ...
    return c.json({ success: true, downloadCount: video.downloadCount });
  } catch (error) {

...
export default function App() {
  ...
  // Wrap everything in ThemeProvider once at the top level
  return (
    <ThemeProvider>
      {loading && (
        <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="mt-4 text-gray-600 dark:text-gray-300">Memuatkan FreeLearning...</p>
          </div>
        </div>
      )}

      {/* Priority 1: Show password reset page if it's a recovery flow */}
      {!loading && isPasswordReset && currentPage === 'reset-password' && (
        <>
          <ResetPasswordPage 
            onBack={() => {
            ...
            onSuccess={() => {
              ...
              setIsPasswordReset(false);
              window.location.hash = '';
              toast.success('Anda kini boleh log masuk dengan kata laluan baharu anda!');
            }}
          />
          <Toaster />
        </>
      ...
      {!loading && !isPasswordReset && user && session && (
        <>
